...
dm.coefs = 0
ngs*tel*dm
tel.computePSF(zeroPaddingFactor=16)
Airy = tel.PSF

indexArray = np.linspace(1, 300, num = 300)
z_coefs = np.random.randn(300)
z_coefs[0:2] = 0

#%%

z_coefs2 = z_coefs  * 100e-9 * indexArray**(-1)
dm.coefs = M2C_zernike @ z_coefs2
ngs*tel*dm
tel.computePSF(zeroPaddingFactor=16)
PSF = tel.PSF

FoV = 1280 / 16
# plt.imshow((Airy ** (1/3)), extent=[-FoV/2, FoV/2, -FoV/2, FoV/2])
# plt.xlim(-10,10)
# plt.ylim(-10,10)

# plt.imshow((PSF ** (1/3)), extent=[-FoV/2, FoV/2, -FoV/2, FoV/2])
# plt.xlim(-10,10)
# plt.ylim(-10,10)

# plt.imshow(((abs(PSF-Airy)) ** (1/3)), extent=[-FoV/2, FoV/2, -FoV/2, FoV/2])
# plt.xlim(-10,10)
# plt.ylim(-10,10)

center = 640
width = 100
plt.plot((Airy[center, center-width:center+width]) ** (1/4), label = 'Airy PSF', linewidth = 3)
plt.plot((PSF[center, center-width:center+width]) ** (1/4), label = 'Aberrated PSF', linewidth = 3)
plt.legend()
